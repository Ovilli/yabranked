...
class EndToEndTest {
    ...
    fun `two players queue, match, and settle`() = runTest {
        ...
        val winner = players.get(match.playerA)!!
        val loser = players.get(match.playerB)!!
        assertTrue(winner.rating > 1000)
        assertTrue(loser.rating < 1000)
        assertEquals(listOf(winner), players.topByRating(1, minMatches = 1))

        // match history visible for both players
        assertEquals(1, matches.historyFor(alice, 10).size)
        assertEquals(1, matches.historyFor(bob, 10).size)
    }
}
