...
class ReplayStream(
    ...
    private val header = ByteBuffer.allocate(FRAME_HEADER)
    ...
    init {
        synchronized(this) {
            out.write(ReplayPacketTap.MAGIC)
            out.write(ReplayPacketTap.FORMAT_VERSION.toInt())
        }
        written.addAndGet(ReplayPacketTap.MAGIC.size + 1L)
    }
    ...
    fun append(protocol: Byte, payload: ByteArray) {
        ...
        synchronized(this) {
            if (closed) return
            if (written.get() + FRAME_HEADER + payload.size > maxBytes) {
                truncated = true
                log.warn(
            ...
            try {
                out.write(header.array(), 0, FRAME_HEADER)
                out.write(payload)
            } catch (e: Exception) {
            ...
            written.addAndGet((FRAME_HEADER + payload.size).toLong())
